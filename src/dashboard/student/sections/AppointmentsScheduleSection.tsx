...
import { 
  ...
  fetchConfirmIndividualAppointment,
  fetchConfirmGroupAppointment,
  fetchCancelIndividualAppointment,
  fetchCancelGroupAppointmentAttendance
} from '@/infrastructure/api/student';
import { 
  formatDate, 
  formatTime, 
  getDayName, 
...
const AppointmentsScheduleSection: React.FC = () => {
  ...
  const loadAppointments = async () => {
    ...
    try {
      ...
      const sent = await fetchAppointmentsSent(studentId);
      setSentAppointments(sent);

      const received = await fetchAppointmentsReceived(studentId);
  ...
  const confirmAppointment = async (appointmentId: number, appointmentModality: string, chosenDateTime: string) => {
    try {
      if (appointmentModality === 'INDIVIDUAL') {
        await fetchConfirmIndividualAppointment(appointmentId, chosenDateTime);
        alert('Cita confirmada exitosamente');
      } else {
        await fetchConfirmGroupAppointment(appointmentId, studentId!, chosenDateTime);
  ...
  const cancelAppointment = async (appointmentId: number, appointmentModality: string) => {
    try {
      if (appointmentModality === 'INDIVIDUAL') {
        await fetchCancelIndividualAppointment(appointmentId);
        alert('Cita cancelada exitosamente');
      } else {
        await fetchCancelGroupAppointmentAttendance(appointmentId, studentId!);
        alert('Asistencia rechazada exitosamente');
      }
      await loadAppointments();
    } catch (err) {
  ...
  const renderAppointmentCard = (
    ...
    const currentStudent = receiverStudents?.find(student => student.studentId === studentId);
    const canConfirm = currentStudent?.attendance === 'SIN_CONFIRMAR';
            ...
            <div className="mt-3 pt-3 border-t border-theme-rich-black/10 space-y-3">
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  await confirmAppointment(apptDetails.id, apptDetails.appointmentModality, selected.value);
                }}
              >
                ✅ Confirmar cita
              </button>
              ...
              <button
                ...
                onClick={() => cancelAppointment(apptDetails.id, apptDetails.appointmentModality)}
              >
                ❌ Cancelar cita
              </button>
            </div>
...
export default AppointmentsScheduleSection;