...
export function getAppointmentStatusInfo(state: string | null | undefined): StatusInfo {
...
export function getAttendanceInfo(attendance: string | null | undefined): AttendanceInfo {
  const attendanceMap: Record<string, AttendanceInfo> = {
    'CONFIRMADA':  { badgeClass: 'bg-green-100 text-green-800 border-green-200', icon: '✅' },
    'RECHAZADA':   { badgeClass: 'bg-red-100 text-red-800 border-red-200', icon: '❌' },
    'SIN_CONFIRMAR': { badgeClass: 'bg-yellow-100 text-yellow-800 border-yellow-200', icon: '⏳' }
  };

  return attendanceMap[attendance || ''] || { badgeClass: 'bg-gray-100 text-gray-800 border-gray-200', icon: '❓' };
}